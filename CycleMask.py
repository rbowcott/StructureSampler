...
def update_state(state, z, done, actions, n):
    ...
    #Update transitive closure of tranpose
    #Reshape goes rows, then columns. Therefore quotient gives row, remainder gives column
    srcs = actions // n  
    targets = actions % n
    src_rows = T.gather(state['closure_T'][~done], 1, srcs.unsqueeze(2).expand(-1, -1, state['closure_T'].size(2)))
    target_cols = T.gather(state['closure_T'][~done], 2, targets.unsqueeze(2).expand(-1, state['closure_T'].size(1), -1))
    state['closure_T'][~done] |= T.logical_and(src_rows, target_cols)
